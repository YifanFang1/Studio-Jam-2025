...
public class Limb : MonoBehaviour
{
    ...
    [Header("Snapping")]
    [Tooltip("When enabled, if the limb gets within this distance of the target it will snap exactly to it to avoid drifting.")]
    public bool snapWhenClose = true;
    [Tooltip("Distance (world units) under which the limb will snap to the target position.")]
    ...
    protected virtual void MovePhysics()
    {
        ...
        Vector2 next;
        if (snapWhenClose && dist <= snapThreshold)
        {
            // Snap exactly to avoid tiny drifting
            next = targetPosition;
        }
        else
        {
            next = Vector2.MoveTowards(currentPos, targetPosition, followSpeed * Time.fixedDeltaTime);
        }

        if (rb != null)
        {
            rb.MovePosition(next);
            // If we snapped, also clear velocity to avoid physics inertia
            if (snapWhenClose && dist <= snapThreshold)
            {
